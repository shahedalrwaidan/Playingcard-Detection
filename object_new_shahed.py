...
# Forth player check
	if fo[-1] == 'c':
		...
		for key, value in D.items(): 
			if value == fo:
				fo_key = key
# Check if the cards contain tarneeb and calculate which one is winner 
...
# IF cards didn't contain tarneeb ... then check using game color (Determinant by the First Card)  
...
while ii <=13 :
	...
	while f:
		...
		# Show rounds, Tarneeb and Input Cards
		l=str(ii)
		...
		cv2.imshow("Image", frame)
		cv2.putText(frame, f"Inputcard :{new_card}", (20, 100),font, 1, (0, 0, 255), 2, cv2.LINE_4)
		cv2.imshow("Image", frame)
